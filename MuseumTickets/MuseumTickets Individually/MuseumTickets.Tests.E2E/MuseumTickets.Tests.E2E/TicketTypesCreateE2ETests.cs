...
public class TicketTypesCreateE2ETests : PageTest
{
    ...
    public async Task Create_Minimal_ShowsInIndex()
    {
        ...
        Assert.That(found, Is.True, $"Tip karte '{ttName}' nije prikazan u listi nakon kreiranja.");
    }
    [Test]
    public async Task Create_WithDescription_ShowsInDetails()
    {
        var museumName = await EnsureMuseumAsync("Novi Sad");
        var ttName = Unique("E2E Tip karte (opis)");
        var desc = new string('x', 200);
        ...
        await FillByLabelAsync("Naziv", ttName);
        await FillByLabelAsync("Cena", "499");
        ...
        await AssertFieldContainsAsync("Cena", "499");
    ...
    public async Task Create_Cancel_DoesNotCreate()
    {
        ...
        var ttName = Unique("E2E Tip karte (cancel)");
        ...
        await FillByLabelAsync("Naziv", ttName);
        await FillByLabelAsync("Cena", "99");
        await FillByLabelAsync("Opis", "ovo NE treba da se sačuva", required: false);
        await SelectOptionByContainsAsync(Page.GetByLabel("Muzej"), museumName);

        await ClickAnyAsync("Otkaži", "Odustani", "Nazad", "Cancel", "Back");
        await Expect(Page).ToHaveURLAsync(new Regex(".*/TipoviKarata"));

        var exists = await RowVisibleInIndexAsync(ttName, retries: 2, delayMs: 400);
        Assert.That(exists, Is.False, $"Tip karte '{ttName}' je kreiran iako je kliknuto otkazivanje.");
    }
}
