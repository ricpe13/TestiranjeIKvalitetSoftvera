...
public class OrdersCreateE2ETests : PageTest
{
    ...
    public async Task Create_Minimal_ShowsInIndex()
    {
        ...
        Assert.That(found, Is.True, $"Porudžbina za '{customer}' nije vidljiva u listi nakon kreiranja.");
    }
    [Test]
    public async Task Create_Details_Displays_Core_Fields()
    {
        var (museum, exhibition, ticket) = await EnsurePrereqsAsync();
        var customer = Unique("E2E Kupac (detalji)");
    ...
    {
        var (museum, exhibition, ticket) = await EnsurePrereqsAsync();
        var customer = Unique("E2E Kupac (cancel)");
        ...
        await FillByLabelAsync("Ime kupca", customer);
        await FillByLabelAsync("Email", email);
        await FillByLabelAsync("Količina", "2");
        await SelectOptionByContainsAsync(Page.GetByLabel("Tip karte"), ticket);
        await SelectOptionByContainsAsync(Page.GetByLabel("Izložba"), exhibition);

        await ClickAnyAsync("Otkaži", "Odustani", "Nazad", "Cancel", "Back");
        await OpenOrdersIndexAsync();

        var exists = await RowVisibleInIndexAsync(customer, retries: 2, delayMs: 400);
        Assert.That(exists, Is.False, $"Porudžbina za '{customer}' je kreirana iako je kliknuto otkazivanje.");
    }
}
