...
public class ExhibitionsCreateE2ETests : PageTest
{
    ...
    private async Task AssertMuseumShownOnDetailsAsync(string? expectedMuseumName = null)
    {
        if (!string.IsNullOrWhiteSpace(expectedMuseumName))
        {
            ...
            {
                await Expect(direct.First).ToBeVisibleAsync();
                return;
            }
        }
    ...
    public async Task Create_Minimal_ShowsInIndex()
    {
        ...
        Assert.That(found, Is.True, $"Izložba '{title}' nije prikazana u listi nakon kreiranja.");
    }
    [Test]
    public async Task Create_WithLongDescription_ShowsInDetails()
    {
        var museumName = await EnsureMuseumAsync("Novi Sad");
        var title = Unique("E2E Izložba (long)");
        var (start, end) = DefaultDates();
    ...
    public async Task Create_Cancel_DoesNotCreate()
    {
        ...
        var title = Unique("E2E Izložba (cancel)");
        var (start, end) = DefaultDates();
        ...
        var exists = await RowVisibleInIndexAsync(title, retries: 2, delayMs: 400);
        Assert.That(exists, Is.False, $"Izložba '{title}' je kreirana iako je kliknuto otkazivanje.");
    }
}
